...
public class WelcomeLoginPage {
	...
	public void show(Stage primaryStage, User user) {
		...
		// Initalizing welcomeScene here to use later
		...
		if ("admin".equalsIgnoreCase(user.getRole())) {
			...
			inviteButton.setOnAction(a -> {
				new InvitationPage().show(databaseHelper, primaryStage, welcomeScene);
			});
			layout.getChildren().add(inviteButton);
		}

		layout.getChildren().addAll(welcomeLabel, continueButton, quitButton);
		// welcomeScene,getChildren().a = new Scene(layout, 800, 400);
